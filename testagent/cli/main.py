...
def help() -> None:
    ...
    typer.echo("")
    typer.echo("── 常用命令 ──────────────────────────────────────")
    typer.echo("  testagent app plan <需求>    根据需求自动生成、执行测试用例并生成报告")
    typer.echo("    -f, --platform           目标平台: android / ios（必填）")
    typer.echo("    -p, --app-package        App 标识 (Android→packageName, iOS→bundleId)")
    typer.echo("    -a, --app-activity       [Android only] 启动 Activity")
    typer.echo("    -d, --device-udid        目标设备序列号")
    typer.echo("")
    ...
    typer.echo("")
    typer.echo("示例:")
    typer.echo('  testagent app plan "测试首页功能" -p com.example.app -y')
