...
def help() -> None:
    ...
    typer.echo("  testagent app plan <需求>    自动生成、执行测试用例并生成报告")
    ...
    typer.echo("    testagent app plan")
    typer.echo("    → 自动扫描 adb devices → 选择设备 → 分配计划 → 并行执行")
    typer.echo("")
    typer.echo("  [2] YAML 配置文件:")
    typer.echo("    testagent app plan -m configs/multi_device.yaml")
    typer.echo("")
    typer.echo("  [3] 传统模式（单设备）:")
    typer.echo('    testagent app plan "需求文档" -p com.example.app -y')
    typer.echo("")
    typer.echo("── 自然语言测试 ──────────────────────────────")
