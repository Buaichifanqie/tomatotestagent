...
App Skill 是针对特定应用的领域知识插件，存放于 `skills/apps/<app_name>/` 目录。与通用 Skill 不同，App Skill 不定义测试流程，而是提供**应用专属的交互知识**，让 Agent 在执行通用 Skill 时能正确操作目标 App。

### 9.1 通用 Skill vs App Skill

| 维度 | 通用 Skill | App Skill |
|------|-----------|----------|
| 存放位置 | `skills/<skill_name>/SKILL.md` | `skills/apps/<app_name>/SKILL.md` + 子文件 |
| 作用 | 定义测试流程骨架 | 提供应用领域知识 |
| 触发方式 | 用户输入匹配 trigger | 目标 App 识别后自动加载 |
| 谁写 | 测试框架开发者 | App 测试人员 |
| 更新频率 | 低（框架级变更） | 高（随 App 版本更新） |
...
skills/apps/
└── bilibili/
    ├── SKILL.md               # 主文件：元信息 + 通用视觉特征 + 核心流程 + 已知陷阱
    ├── search_flow.md         # 子文件：搜索功能专项
    └── video_playback.md      # 子文件：视频播放专项
```

**主文件 SKILL.md** 必须存在，包含：
- YAML Front Matter（元信息、触发词、依赖）
- 视觉特征库（各页面的关键 UI 元素描述）
- 核心业务流（冷启动、弹窗处理、主要功能入口）
- 已知陷阱（Agent 容易犯的错误）

**子文件** 按功能拆分，每个子文件聚焦一个功能域。子文件通过 `parent` 字段关联主文件。

### 9.3 主文件 SKILL.md 格式

```yaml
---
name: bilibili                          # App 名称（小写英文）
version: "1.0.0"                        # 语义化版本
description: 哔哩哔哩 App 测试技能       # 短描述
app_info:
  package_name: "tv.danmaku.bili"       # Android 包名
  launch_activity: ".MainActivityV2"    # 启动 Activity
  login_method: "sms"                   # 登录方式
  platforms: [android, ios]             # 支持平台
triggers:                               # App 识别触发词
  - "bilibili"
  - "哔哩哔哩"
  - "B站"
  - regex: "tv\\.danmaku\\.bili"        # 正则匹配包名
dependencies:
  skills: [app_smoke_test]              # 依赖的通用 Skill
  mcp_servers: [appium, vision]         # 依赖的 MCP Server
tags: [video, social]                   # 标签
---
```
...
按页面组织，每个元素用**视觉描述**而非 resource-id：

```markdown
### 首页
- **底部导航栏**：4 个 Tab，从左到右「首页」「动态」「消息」「我的」
  - 选中态：图标变为粉色，文字变为粉色
  - 未选中态：灰色图标 + 灰色文字
- **顶部搜索栏**：右侧有放大镜图标，灰色背景圆角矩形
```

**为什么用视觉描述？** Agent 通过截图（Vision MCP）识别元素，resource-id 在截图中不可见，视觉特征才是 LLM 能理解的语言。
...
**关键原则：写时序，不只写状态。** "播放页有全屏按钮"是状态描述，Agent 不知道怎么到达那个状态。"点击视频区域 → 控制栏浮现 → 点击全屏按钮"才是 Agent 能执行的操作序列。

#### 已知陷阱（必写）

列出 Agent 容易犯的错误和反直觉的行为：

```markdown
### 已知陷阱
- **控制栏默认隐藏**：播放页看不到全屏/播放按钮，必须先点击视频区域
- **控制栏自动消失**：浮现后几秒无操作即隐藏
- **搜索历史弹窗遮挡**：需先关闭弹窗才能操作搜索框
```

**写法要求：** 每条陷阱写清「现象」和「正确做法」，不要只写现象。"控制栏默认隐藏"是现象，"必须先点击视频区域"是正确做法。

### 9.4 子文件格式

```yaml
---
parent: bilibili                        # 关联的主文件名
version: "1.0.0"
description: 视频播放功能专项测试
trigger: "视频|播放|全屏|倍速"           # 功能级触发词
---
```
...
子文件比主文件更详细。主文件写概要（"控制栏隐藏，需点击视频区域"），子文件写完整操作序列（每一步该点哪里、等什么、验证什么）。

### 9.5 编写检查清单

写完一个 App Skill 后，逐项检查：

**主文件 SKILL.md：**
- [ ] `app_info.package_name` 填写正确（Android 包名）
- [ ] `triggers` 覆盖中文名、英文名、包名
- [ ] 视觉特征库覆盖主要页面（首页、核心功能页）
- [ ] 核心业务流写清了交互时序（不只是状态描述）
- [ ] 已知陷阱写了「现象 + 正确做法」
- [ ] 弹窗处理规则完整（青少年模式、版本更新、权限弹窗等）

**子文件：**
- [ ] `parent` 字段正确指向主文件名
- [ ] 视觉特征库覆盖该功能域的所有关键 UI 元素
- [ ] 操作流程写清了每一步的前置条件（如"控制栏浮现后才能…"）
- [ ] 已知陷阱包含隐藏状态、自动消失、时序依赖等反直觉行为

### 9.6 常见错误

| 错误写法 | 正确写法 | 原因 |
|---------|---------|------|
| "播放页有全屏按钮" | "点击视频区域 → 控制栏浮现 → 全屏按钮在控制栏最右侧" | Agent 不知道怎么到达目标状态 |
| "resource-id: btn_fullscreen" | "控制栏最右侧，图标为四个角向外的箭头" | Agent 通过截图识别，看不到 resource-id |
| "搜索栏在顶部" | "首页顶部灰色圆角矩形，右侧放大镜图标" | 多模态 LLM 需要视觉特征才能定位 |
| "点击播放按钮" | "点击视频区域浮现控制栏 → 点击中央的 ▶ 图标" | 播放按钮默认隐藏，需要先触发 |
| 不写已知陷阱 | 列出每个 Agent 可能犯的错 + 正确做法 | Agent 会反复踩同一个坑 |
